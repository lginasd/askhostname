...
impl NbnsRequest {
    ...
    let remote: SocketAddr = match format!("{}:{}", addr, NbnsRequest::PORT).parse() {
        Ok(a) => a,
        Err(e) => {
    ...
    let buff: Vec<u8>;
    sock.set_read_timeout(Some(std::time::Duration::from_millis(NbnsRequest::TIMEOUT_MS))).unwrap();
    ...
    let (_, response) = buff.split_at(NbnsRequest::SIZE + 4);
